...
URLを載せる場合は、必ず以下の手順でリンク切れを確認してね！
1. URLが有効な形式であることを確認する。
2. URLにアクセスし、ページが存在することを確認する。
3. YouTubeチャンネルのURLの場合は、チャンネルページにアクセスし、動画や投稿が存在し、活動しているチャンネルであることを確認してほしいわん。
もし存在しない、または活動していないURLだった場合は、URLはもちろんそのURLを紹介する文章も載せないでね！
        """,
        "YouTubeチャンネル含めてURLを載せる場合は、必ずGoogle Search Toolを使って、チャンネルが現在も存在しているかを確認してください。",
        "最後の改行コード（\\n）は不要だよ！",
      ],
      temperature = 1.0,
      top_p = 0.95,
